...
| 2 | リファクタリング | (必要に応じて)コード整理、レビュー、動作確認（詳細は2-1〜2-5） | 動作確認OK | Copilot/ユーザー（詳細は下記サブステップ参照） |
| 2-1 | リファクタ指示 | リファクタを実施する箇所・目的を指示 | 指示記録済み | ユーザー |
| 2-2 | リファクタ仕様検討 | Copilotが提案するリファクタ仕様を作成 | 仕様案作成 | Copilot |
| 2-3 | リファクタ実装 | Copilotがリファクタを実装する | 実装完了 | Copilot |
| 2-4 | リファクタ単体テスト・承認 | 単体テスト作成・実行 | テスト通過 | Copilot |
| 2-5 | 結果確認 | ユーザーがリファクタ結果を確認・承認 | ユーザー確認OK | ユーザー |
| 3 | コード・記事作成 | 機能実装、ブログ記事作成、自動テスト作成（詳細は3-1〜3-4） | テスト通過 | Copilot/ユーザー（詳細は下記サブステップ参照） |
| 3-1 | 要件確認 | 既に記載済みの要件を確認し、必要な追記・修正を洗い出す | 要件確定 | ユーザー |
| 3-2 | 実装・記事下書き | Copilotがコード生成・記事下書きを作成 | 下書き作成 | Copilot |
| 3-3 | 実装セルフレビュー（自動チェック） | Copilotが自動テスト・基本動作のセルフチェックを実行 | 自動チェック実行済み | Copilot |
| 3-4 | 実装レビュー・承認 | ユーザーがセルフレビュー結果を確認し承認する | テスト通過・承認済み | ユーザー |
...
| 5 | 動作確認 | アプリケーション実行、動作テスト（詳細は5-1〜5-4） | 動作確認OK | Copilot/ユーザー（詳細は下記サブステップ参照） |
| 5-1 | 動作確認手順作成 | Copilotが実行手順とテストケースを提示 | 手順提示済み | Copilot |
| 5-2 | 実行・検証 | ユーザーがアプリケーションを実行し検証 | 動作確認OK | ユーザー |
| 5-3 | 追加クラス単体テスト作成・実行 | Copilotが5で追加したクラスに対する単体テストを作成・実行 | テスト作成・実行済み | Copilot |
| 5-4 | 追加クラステスト結果確認 | ユーザーがテスト結果を確認・承認する | 結果確認済み | ユーザー |
...
- **タグ付け・マージ**: 記事公開後、以下の順序で実施
  ...
  ```
- **プッシュ時のHTTP 400エラー対策**: 画像等大きなファイルを含む場合、バッファサイズを増やす
  ```bash
