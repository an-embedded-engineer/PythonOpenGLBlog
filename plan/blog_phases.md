...
| 4a    | シェーダー基礎（前編）    | 第4回「シェーダー入門（頂点編）」         |
| 4b    | シェーダー基礎（後編）    | 第5回「シェーダー入門（フラグメント編）」 |
| 4c    | はじめてのポリゴン描画    | 第6回「虹色の三角形を描く」               |
| 5     | 座標変換とカメラ          | 第7回「カメラと座標変換」                 |
...
| 4a    | `phase4a/shader-vertex`    |
| 4b    | `phase4b/shader-fragment`  |
| 4c    | `phase4c/first-polygon`    |
